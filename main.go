...
var boxes []uint // for the drawing of walls FIXME should not be global imo
var width, height int = 40,40
...
func make_2d_int(h,w int) [][]int {
	var param [][]int = make([][]int, h)
	for i := range param {
		param[i]=make([]int, w)
	}
	return param
...
func make_2d_wall_s(h, w int) [][]*wall_s {
...
type wall_s struct {
	x, y, vertical, active int
}

func new_wall_s(x, y, vertical, active int) *wall_s {
...
func prims() {
	...
	for (wall_list.Len()>0) {
		rand.Seed( time.Now().UTC().UnixNano())
		rand_wall = rand.Intn(wall_list.Len())
		ctr=0
...
func wall(b []uint, o, offset, a_cord cord, i int, a float64, s size) []uint {
...
func init_walls() {
	...
	var x_ctr int = 0	// x ctr for walls
	var y_ctr int = 0   // y ctr for walls
	...
	for _, x := range v_walls {
		for y := range x {
			...
			if (x[y].active == 1) {
				...
				boxes = wall(boxes,origin_disp, boff, cord{0,0,0},ctr,0, my_size)    // top
				ctr++
			}
			y_ctr++
		}
	...
	for _, x := range h_walls {
		for y := range x {
			...
			if (x[y].active == 1) {
				...
				boxes = wall(boxes,origin_disp, boff, cord{1,0,0},ctr,90, my_size) // left
				ctr++
			}
			y_ctr++
		}
